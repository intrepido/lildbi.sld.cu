...
class DocumentsController extends AppController {
	...
	function add() { //FUNCIONA el adicionar un documento (Hay que especificar un squema en el modelo)
		...
		elseif (isset($this->params['pass'][0])){
			if ($this->params['pass'][0] == 'sin_indizacion') {
				$this->Session->write('index', __('Sin Indezación'));
			}
			else
			{
				$this->Session->write('index', __('Con Indezación'));
			}			
		}
	...
	function visualization(){ //Boton "Vista Previa" de la pagina "series_monograficas"
			...
			$arrayData = array_merge(array('v93' => array(__('Fecha de ultima modificación') => CakeTime::format('Ymd', time()))), $arrayData);
			
			if($this->Session->check('dateCreationEdit')){//si se esta modificando
				$arrayData = array_merge(array('v91' => array(__('Fecha de Creación del Registro') => $this->Session->read('dateCreationEdit'))), $arrayData);
			}else{//si se esta insertando
				$arrayData = array_merge(array('v91' => array(__('Fecha de Creación del Registro') => CakeTime::format('Ymd', time()))), $arrayData);
			}
			
			$arrayData = array_merge(array('v899' => array(__('Versión del Software') => 'LILDBI-WEB 1.8')), $arrayData);
			$arrayData = array_merge(array('v1' => array(__('Código del Centro') => 'CU1.1')), $arrayData);
